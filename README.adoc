...
- For current countries there are link:i18n-regions-countries/src/main/java/org/meeuw/i18n/countries/CurrentCountry.java[`org.meeuw.i18n.countries.CurrentCountry`]s. Backed by  `org.meeuw.i18n.countries.codes.CountryCode` (used to be `com.neovisionaries.i18n.CountryCode`)
...
NOTE: I considered https://github.com/TakahikoKawasaki/nv-i18n abandoned. For now just the CountryCode enum was copied (to a different package), and the dependency was dropped.  May be it can also be auto created using online resources. Or we use the list provided in java 9's `java.util.Locale`. Or perhaps https://github.com/detrin/download-iso3166-list could help.
...
`org.meeuw.i18n.subdivisions.codes.CountrySubdivisionCode` (from https://github.com/mihxil/i18n-subdivisions)
...
| 3.x
| drop dependency on neovisionaries. This gives a few backwards compatibility issues
  - `com.neovisionaries.i18n.CountryCode` from nv-i18n was the most important reason to use this library. It is now replaced by `org.meeuw.i18n.countries.codes.CountryCode`. This is for now just a copy. In the future it might be generated from an online resource.
| oktober 2025
...
| 3.3
| Moved away from checkerframework, which was only very lightly used (basically just @Nullable/@NonNull). We now do jspecify, following the example of other projects like guava.
|
